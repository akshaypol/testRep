                                    ...
                                    <div id="LDiv">
                                        <div class="table-responsive">
                                            <table id="example" class="table table-bordered">
                                                ...
                                                <tbody>
                                                    <%                                                        List<WithDrawModel> mm = new WithDrawDao().getByStatus();
                                                        int g = 0;
                                                        for (WithDrawModel m : mm) {
                                                            g++;
                                                            MemberModel me = new MemberDao().getById(m.getUserID());
                                                            if(m.isStatus() || m.isMoneyReturned()){
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><%=m.getWamount()%></td>
                                                        <td> 
                                                            <a href="../PaidUser?MId=<%=m.getUserID()%>&Amount=<%=m.getWamount()%>&WMID=<%=m.getId()%>" class="btn-sm btn btn-primary shadow-primary btn-round  waves-effect waves-light">Paid</a>
                                                            <a href="../NotPaidUser?MId=<%=m.getUserID()%>&Amount=<%=m.getWamountMain()%>&WMID=<%=m.getId()%>" class="btn-sm btn btn-danger shadow-primary btn-round waves-effect waves-light">Not Paid(Cancel)</a>
                                                        </td>
